...
void run_parent(pid_t pid) {
	...
	for ( ; ; ) {
		...
		if (WIFEXITED(stat)) {
			...
			long long used_time = res.ru_utime.tv_usec;
			long long used_mem  = res.ru_maxrss;
			if (used_time / 1000.0 > utm_lmt)
				cerr("{\"status\": \"XCPU\"}\n");
			else if (used_mem > mem_lmt / 1024.0)
				cerr("{\"status\": \"XMEM\"}\n");
			else
				cerr("{\"status\": \"OK\", \"time\": %lld, \"mem\": %lld}\n", used_time, used_mem);
			break;
		}
