...
(function(){
  var async, util, fs, childProcess, tmp, bluebird, co, debug, path, coLimiter, _, config, log, asyc, compile, flattenDir, upload, deleteTestData, uploadImage, genDataPairs, testlibExitcodes, dropFirstLine, judgeResult, limit, runAtom, calcProbScore, judge, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice;
  async = require('async');
  util = require('util');
  fs = require('mz/fs');
    ...
    walk = function*(dir){
      ...
      for (i$ = 0, len$ = files.length; i$ < len$; ++i$) {
        ...
        if (fs.statSync(file).isDirectory()) {
          ...
          results$.push(fs.rmdir(dir));
        } else if (dir !== baseDir) {
          results$.push(fs.renameSync);
        }
      }
      return results$;
    };
  ...
  out$.deleteTestData = deleteTestData = co.wrap(function*(pid, filename){
    var filePath;
    filename = path.basename(filename);
    filePath = path.join(config.dataDir, "/" + pid + "/", filename);
    log("delete " + filePath);
    return (yield fs.unlink(filePath));
  });
  out$.uploadImage = uploadImage = co.wrap(function*(part){
    var extName, imageFile, baseName;
    extName = path.extname(part.filename);
    imageFile = tmp.fileSync({
      postfix: extName,
      prefix: "",
      dir: config.imageDir,
      keep: true
    }).name;
    log("image upload " + part.filename + " -> " + path.resolve(imageFile));
    part.pipe(fs.createWriteStream(path.resolve(imageFile)));
    baseName = path.basename(imageFile);
    log(imageFile, baseName);
  ...
  judgeResult = co.wrap(function*(pid, inFile, outFile, ansFile, cfg){
    ...
    try {
      ...
      e = e$;
      messages = dropFirstLine(e.message);
      return {
  ...
  calcProbScore = function(results){
    ...
    for (i$ = 0, len$ = results.length; i$ < len$; ++i$) {
      ...
      if (result.space) {
        ret.space >= (ref$ = result.space) || (ret.space = ref$);
      }
