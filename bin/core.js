...
(function(){
  var async, util, fs, childProcess, tmp, bluebird, co, debug, path, coLimiter, _, config, log, asyc, compile, flattenDir, upload, uploadImage, genDataPairs, testlibExitcodes, dropFirstLine, judgeResult, limit, runAtom, calcProbScore, judge, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice;
  async = require('async');
  util = require('util');
  fs = require('mz/fs');
  ...
  out$.judge = judge = co.wrap(function*(lang, code, probConfig, doc){
    var tmpDir, config, pid, exePath, e, message, result, dataset, ref$, results, i$, len$, data, res, err, ret;
