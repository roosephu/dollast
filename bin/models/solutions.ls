...
export model = conn.model \solution, schema

log = debug \dollast:sol

next-count = make-next-count model, 1

export submit = (req, uid) ->*
  # @ensure-access model, 0, \x # sol = 0 => submision
  sol = new model do
    _id: yield next-count!
    code: req.code
    lang: req.lang
    prob: req.pid
    user: req.permit.owner
    final:
      status: \running
    permit: req.permit

  prob = yield db.problems.model.find-by-id req.pid, \config .exec!
  if not prob
