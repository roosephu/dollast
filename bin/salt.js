// Generated by LiveScript 1.3.1
var nodeForge, crypto, debug, _, log, privKey, RSA, ref$, out$ = typeof exports != 'undefined' && exports || this;
nodeForge = require('node-forge');
crypto = require('crypto');
debug = require('debug');
_ = require('prelude-ls');
log = debug("dollast:salt");
privKey = '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----';
RSA = nodeForge.pki.privateKeyFromPem(privKey);
ref$ = out$;
ref$.unsalt = function(pswd, ctx){
  var saltedPswd, ref$, salt;
  log(pswd);
  saltedPswd = RSA.decrypt(nodeForge.util.decode64(pswd));
  ref$ = _.splitAt(16, saltedPswd), salt = ref$[0], pswd = ref$[1];
  log(saltedPswd, salt, pswd);
  if ((ctx != null ? (ref$ = ctx.session) != null ? ref$.salt : void 8 : void 8) == null || salt !== ctx.session.salt) {
    throw new Error("wrong salt detected");
  }
  return pswd;
};
ref$.genSalt = function(){
  var bytes;
  bytes = crypto.randomBytes(8);
  return bytes.toString('hex');
};